...
def updateBestRecipesList(recipe: Recipe):
    ...
    if (recipe.profit > least_best_recipes_profit):
        if (best_recipes_count == 1):
            ...
            best_recipes_by_profit.sort(key=lambda recipe: (recipe.profit),
                                        reverse=True)
            best_recipes_by_profit = best_recipes_by_profit[:
                                                            best_recipes_count]
    ...
    if (recipe.sell_price > least_best_recipes_sell_price):
        if (best_recipes_count == 1):
            ...
            best_recipes_by_sell_price.sort(key=lambda recipe:
                                            (recipe.sell_price),
                                            reverse=True)
            best_recipes_by_sell_price = best_recipes_by_sell_price[:
                                                                    best_recipes_count]
            least_best_recipes_sell_price = best_recipes_by_sell_price[
                -1].sell_price
        if (verbose):
